$(document).ready(function () {
    ...
    window.list_operation_all = function () {
        ...
        if (params.has("filtre")) {
            let filtreParam = params.get("filtre"); // Peut être "Versement" ou "5"

            // Vérifier si c'est déjà une value
            let optionByValue = $('#filtre option[value="' + filtreParam + '"]');

            if (optionByValue.length > 0) {
                // Cas 1 : on a trouvé une option par sa value
                filtre = filtreParam;
                $('#filtre').val(filtre).trigger('change.select2');
            } else {
                // Cas 2 : essayer de matcher par texte
                let optionByText = $('#filtre option').filter(function () {
                    return $(this).text().trim() === filtreParam;
                });

                if (optionByText.length > 0) {
                    filtre = optionByText.val();
                    $('#filtre').val(filtre).trigger('change.select2');
                } else {
                    // Cas 3 : rien trouvé → fallback
                    filtre = $('#filtre').val();
                }
            }
        } else {
        ...
        $.ajax({
            url: url_base + '/api/list_operation_all/'+date1+'/'+date2+'/'+user.magasin_id+'/'+filtre,
            method: 'GET',
            dataType: 'json',
                    ...
                    $.each(operation, function(index, item) {
                        ...
                        row += `
                            <tr class="nk-tb-item">
                                ...
                                <td class="nk-tb-col">
                                    <span class="badge ${item.type_operation == '4' || item.type_operation == '5' ? 'bg-secondary' : (item.type_operation == 6 || item.type_operation == 7 ? 'bg-warning' : (item.type == 'sortie' ? 'bg-danger' : (item.type == 'entree' ? 'bg-success' :'bg-secondary' )))}">
                                        ${item.type_operation == '4' ? 'Ouverture de caisse' : (item.type_operation == '5'   ? 'Fermeture de caisse' : (item.type_operation == 6 || item.type_operation == 7 ? 'Transfert de recettes' : (item.type == 'entree' ? 'Entrer d\'argent' : (item.type == 'sortie' ? 'Sortie d\'argent' :'Systéme' ))))}
                                    </span>
                                </td>
                                <td class="nk-tb-col">
                                    <span class="badge ${item.type == 'sortie' ? 'bg-danger' : (item.type == 'entree' ? 'bg-success' : (item.type_operation == '4' || item.type_operation == '5' ? 'bg-secondary' : (item.type_operation == 6 || item.type_operation == 7 ? 'bg-warning' : 'bg-secondary' )))}">
                                        ${(item.type == 'sortie' || item.type_operation == 6 ? '-' : (item.type == 'entree' || item.type_operation == 7 ? '+' : ''))} 
                                ...
                                <td class="nk-tb-col">
                                    <ul class="nk-tb${tableBtnPosition == 1 ? `-actions` : ``} gx-1 d-flex">
                                        <li class="nk-tb${tableBtnPosition == 1 ? `-action-hidden` : ``}" >
                                            <a  href="#"
    ...
    $('.table_operation').on('click', '.btn-detail', function (event) {
        ...
        const uniqueId = "modalDetail_" + Date.now(); // ID unique
        ...
        let solde;
        if (type === 'entree' || typeop == 7) {
            solde = '+ ' + montant.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') + ' Fcfa';
        } else if (type === 'sortie' || typeop == 6) {
            solde = '- ' + montant.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') + ' Fcfa';
        } else {
            solde = montant.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') + ' Fcfa';
        }
        ...
        } else if (typeop == 6 || typeop == 7) {
            op = "Transfert de recettes";
        }

        // Injecter les données dans le bon modal
        $(`#${uniqueId} #d_creer_par`).text('Créer par ' + login);
        $(`#${uniqueId} #d_datecreat`).text('Le ' + formatDate(created_at));
        $(`#${uniqueId} #d_type`).text(op);
        $(`#${uniqueId} #d_motif`).text(libelle);
        $(`#${uniqueId} #d_montant`).text(solde);
        $(`#${uniqueId} #d_magasin`).text(magasin);
        $(`#${uniqueId} #d_dateop`).text('Le ' + formatDateHeure(created_at));
        $(`#${uniqueId} #d_num`).text(numop ?? 'Aucun');
    });
    ...
    $("#filtre").on('change', function (event) {
        ...
        // Supprimer le paramètre "filtre" de l'URL
